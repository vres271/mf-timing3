...
export class TimingService {
                ...
                switch (tcMessage?.command) {
                    ...
                    case TimecontrolOutputCommand.LAP:
                        ...
                        if (this.timer?.lap < this.timer?.lapsCount) {
                            this.audioService.playFile('beep_lap.mp3', 0, .6, false);
                            this.audioService.playFile(`lap_${this.timer?.lap}.mp3`, 800, 1, true);
                        }
                        if (this.timer?.lap === this.timer?.lapsCount - 1) {
                            this.audioService.playFile('last.mp3', 1800, 1, true);
                        }
                        break;
                    case TimecontrolOutputCommand.FINISH:
                        this.audioService.clearPending();
                        this.timer.registerFinish(tcMessage.time || 0);
                        this.state = TimingState.Ready;
                        this.addRaceEvent(tcMessage, RaceEventType.Finish);
                        this.audioService.playFile('beep_finish.mp3', 0, 1, false);
                        this.audioService.playFile('finish.mp3', 500, 1, true);
    ...
    private playPersonalSound(tcMessage: TimecontrolOutputDTO) {
        ...
        if (tcMessage?.command === TimecontrolOutputCommand.START ||
            tcMessage?.command === TimecontrolOutputCommand.LAP) {
            ...
            if (reasonToPlay && this.timer?.lap < this.timer?.lapsCount) {
                if (racer?.userFullName && /Шатурн/.test(racer.userFullName)) {
                    ...
                    if (randomIndex === this.lastIndexPersonalSoundPlayed && soundFileNames.length > 1) {
                        randomIndex = (randomIndex + 1) % soundFileNames.length;
                    }
                    this.lastIndexPersonalSoundPlayed = randomIndex;
                    this.audioService.playFile(soundFileNames[randomIndex], 2500, 1, true);
